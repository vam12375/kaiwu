...
将浏览器标签页 favicon 设置为 `kaiwu/public/logo2.png`，让页面标签左侧显示项目提供的品牌图标，而不是浏览器默认图标。

## What I already know

* 用户指定使用 `kaiwu/public/logo2.png` 作为 favicon。
* `kaiwu/index.html` 当前没有 favicon `link` 声明。
* `kaiwu/public/` 是 Vite 静态资源目录，资源可用根路径 `/logo2.png` 引用。
...
* favicon 指向现有静态资源 `logo2.png`。
* 不触碰无关后端配置和已有未提交改动。

## Acceptance Criteria

* [ ] `kaiwu/index.html` 包含指向 `/logo2.png` 的 favicon 声明。
* [ ] 前端生产构建通过。
